...
func TestDiviRxHHLL(t *testing.T) {
	...
	a.Errorf(
		Eval(v, vm.Opcode(0xA0000000)),
		"Division by zero didn't return an error",
	)

	for _, test := range divTestCases {
		v.Regs[2] = test.x
		o := vm.Opcode(0xA0020000).WithHHLL(uint16(test.y))
		if a.NoError(Eval(v, o)) {
...
func BenchmarkDiviRxHHLL(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		if err := diviRxHHLL(v, vm.Opcode(n|0x0100)); err != nil {
			b.Fatal(err)
		}
	}
}
...
func TestDivRxRy(t *testing.T) {
	...
	for _, test := range divTestCases {
		...
		if a.NoError(Eval(v, vm.Opcode(0xA1420000))) {
			checkOpResults(a, &test, v.Regs[2], v.Flags, "/")
			a.Equalf(vm.Pointer(vm.RAMStart), v.PC, "PC shouldn't move")
			a.Equalf(vm.Pointer(vm.StackStart), v.SP, "SP shouldn't move")
		}
...
func BenchmarkDivRxRy(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		if err := divRxRy(v, vm.Opcode(n&0xFFF0FFFF)); err != nil {
			b.Fatal(err)
		}
		// Reset R0 to 1
		v.Regs[0] = 1
...
func TestDivRxRyRz(t *testing.T) {
	...
	for _, test := range divTestCases {
		...
		if a.NoError(Eval(v, vm.Opcode(0xA2420100))) {
...
func BenchmarkDivRxRyRz(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		// Write results only in R0
		if err := divRxRyRz(v, vm.Opcode(n&0xFFFFF0FF)); err != nil {
			b.Fatal(err)
		}
		// Reset R0 to 1
		v.Regs[0] = 1
...
func TestModiRxHHLL(t *testing.T) {
	...
	a.Errorf(
		Eval(v, vm.Opcode(0xA3000000)),
		"Division by zero didn't return an error",
	)

	for _, test := range modTestCases {
		v.Regs[2] = test.x
		op := vm.Opcode(0xA3020000).WithHHLL(uint16(test.y))
...
func BenchmarkModiRxHHLL(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		// Avoid division by 0 (n|1)
		if err := modiRxHHLL(v, vm.Opcode(n|0x0100)); err != nil {
			b.Fatal(err)
		}
	}
}
...
func TestModRxRy(t *testing.T) {
	...
	for _, test := range modTestCases {
		...
		if a.NoError(Eval(v, vm.Opcode(0xA4420000))) {
			checkOpResults(a, &test, v.Regs[2], v.Flags, "MOD")
			a.Equalf(vm.Pointer(vm.RAMStart), v.PC, "PC shouldn't move")
			a.Equalf(vm.Pointer(vm.StackStart), v.SP, "SP shouldn't move")
		}
...
func BenchmarkModRxRy(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		// Write results only in R0
		if err := modRxRy(v, vm.Opcode(n&0xFFF0FFFF)); err != nil {
			b.Fatal(err)
		}
		// Reset R0 to 1
		v.Regs[0] = 1
...
func TestModRxRyRz(t *testing.T) {
	...
	for _, test := range modTestCases {
		...
		if a.NoError(Eval(v, vm.Opcode(0xA5420100))) {
...
func BenchmarkModRxRyRz(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		// Write results only in R0
		if err := modRxRyRz(v, vm.Opcode(n&0xFFFFF0FF)); err != nil {
			b.Fatal(err)
		}
		// Reset R0 to 1
		v.Regs[0] = 1
...
func TestRemiRxHHLL(t *testing.T) {
	...
	a.Errorf(
		Eval(v, vm.Opcode(0xA6000000)),
		"Division by zero didn't return an error",
	)

	for _, test := range remTestCases {
		v.Regs[2] = test.x
		op := vm.Opcode(0xA6020000).WithHHLL(uint16(test.y))
...
func BenchmarkRemiRxHHLL(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		// Avoid division by 0 (n|1)
		if err := remiRxHHLL(v, vm.Opcode(n|0x0100)); err != nil {
			b.Fatal(err)
		}
	}
}
...
func TestRemRxRy(t *testing.T) {
	...
	for _, test := range remTestCases {
		...
		if a.NoError(Eval(v, vm.Opcode(0xA7420000))) {
			checkOpResults(a, &test, v.Regs[2], v.Flags, "%")
			a.Equalf(vm.Pointer(vm.RAMStart), v.PC, "PC shouldn't move")
			a.Equalf(vm.Pointer(vm.StackStart), v.SP, "SP shouldn't move")
		}
...
func BenchmarkRemRxRy(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		// Write results only in R0
		if err := remRxRy(v, vm.Opcode(n&0xFFF0FFFF)); err != nil {
			b.Fatal(err)
		}
		// Reset R0 to 1
		v.Regs[0] = 1
...
func TestRemRxRyRz(t *testing.T) {
	...
	for _, test := range remTestCases {
		...
		if a.NoError(Eval(v, vm.Opcode(0xA8420100))) {
			checkOpResults(a, &test, v.Regs[1], v.Flags, "%")
			a.Equalf(vm.Pointer(vm.RAMStart), v.PC, "PC shouldn't move")
			a.Equalf(vm.Pointer(vm.StackStart), v.SP, "SP shouldn't move")
		}
...
func BenchmarkRemRxRyRz(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		// Write results only in R0
		if err := remRxRyRz(v, vm.Opcode(n&0xFFFFF0FF)); err != nil {
			b.Fatal(err)
		}
		// Reset R0 to 1
		v.Regs[0] = 1
